...
def get_random_next_mode_name(current_mode_name: str, available_modes_name: list = None):
    ...
    if available_modes_name is None:  # 没有指定模式列表时，使用所有待机模式
        from image_modes import modes_standby_fix, modes_standby_move,modes_standby
        # 如果启用了抛掷功能且不是鼠标跟随模式，则添加移动模式
        if config.throw_follow_enabled and  not config.mouse_follow_enabled :
            modes = modes_standby
        else:
...
class TimerNextChange(NextChangeMode):
    """定时器下一个行为"""
    title: str = "定时切换类"
    def __init__(self, widget: QWidget, time_next_enabled: bool = True, interval_min=3000, interval_max=4000, prob=0.5):
        super().__init__(widget)
