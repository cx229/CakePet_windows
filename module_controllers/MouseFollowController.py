...
class MouseFollowController(ModuleController):
    ...
    def throw_begin(self):
        ...
        self.widget.mode_manager.set_mode(image_modes.ThrowMode.name())  # 切换到抛掷模式
    ...
    def func_mouse(self):
        ...
        if not key_monitor.check_key(keyboard.Key.ctrl_l, is_only=True):
    ...
    def follow_update(self):
        ...
        """
        try:
            if config.is_drag_follow:  # 拖动时，不跟随
                return
            if config.throw_follow_enabled and config.is_throw_follow: # is_throw_follow 为 True 时，执行抛掷跟随（拖动结束时开，拖拽结束关）
                self.func_throw()
